...
class Model_orders extends CI_Model
{
	...
	public function create()
	{
		...
		$bill_no = 'SO-'.strtoupper(substr(md5(uniqid(mt_rand(), false)), 0, 3));
    	$data = array(
    		'bill_no' => $bill_no,
    	...
    	if($this->input->post('product'))
        	$data['type'] = 1;
        else if($this->input->post('service'))
        	$data['type']  = 2;
        ...
        $count_service = count($this->input->post('service'));
    	...
    	for($x = 0; $x < $count_service; $x++) {
    		$items = array(
    			'order_id' => $order_id,
    			'service_id' => $this->input->post('service')[$x],
    			'qty' => $this->input->post('qty')[$x],
    			'rate' => $this->input->post('rate_value')[$x],
    			'amount' => $this->input->post('amount_value')[$x],
    		);
    		...
    		// now decrease the stock from the product
    		$product_data = $this->model_services->getServiceData($this->input->post('service')[$x]);
    		$qty = (int) $product_data['qty'] - (int) $this->input->post('qty')[$x];

    		$update_product = array('qty' => $qty);


    		$this->model_services->update($update_product, $this->input->post('service')[$x]);
    	}
